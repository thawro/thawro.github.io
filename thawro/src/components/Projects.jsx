...
const ProjectCard = ({ index, name, description, tags, image, urls }) => {
  return (
    <motion.div
      ...
      >
        <div className='relative w-full h-[230px]'>
          ...
          />
          {/* <gradio-app src={hf_url}></gradio-app> */}
          <div className='absolute inset-0 flex justify-end m-3 card-img_hover'>
            {console.log(urls)}
            {urls.map((url, index) => (
              <a
                href={url.url}
                className='mr-2 pulsating mt-2 bg-tertiary border-2 border-indigo-300/50 w-10 h-10 rounded-full flex justify-center items-center cursor-pointer'
              >
                <img
                  src={url.icon}
                  alt={url.icon}
